...
**Feature Branch**: `004-context-menu-create-notebook`
...
1. **Given** a folder `workbooks` exists in the vault, **When** the user right-clicks the folder and selects "Create new marimo notebook", **Then** a new file `workbooks/untitled_marimo.py` is created with the marimo template and opened in the editor.
...
- **FR-001**: The plugin MUST register a handler for Obsidian's `file-menu` event to add context menu items.
- **FR-002**: A menu option "Create new marimo notebook" (icon: `plus` or similar) MUST be added to the context menu when right-clicking folders and files in the file explorer.
- **FR-003**: The plugin MUST determine the target creation directory:
  ...
  - If no clear parent folder can be resolved, fall back to the vault root.
- **FR-004**: Selecting the menu option MUST create a new notebook file named `untitled_marimo.py` (or sequential name if colliding) containing the standard `NEW_NOTEBOOK_TEMPLATE` code.
- **FR-005**: The newly created notebook MUST be opened immediately in a new marimo editor leaf (using the `openMarimo` method).

### Key Entities

- **Context Menu Item**: The UI element labeled "Create new marimo notebook" added to the Obsidian context menu.
- **Target Folder**: The destination folder resolved from the right-clicked file or folder (`TAbstractFile`).
...
- Desktop-only environment where standard Obsidian context menu APIs and file creation function as expected.
